...
static bool
exportFaceGroupToShape(	//psdeo: returns if anything is present in facegroup
  shape_t& shape,
  const std::vector<float> &in_positions,
  const std::vector<float> &in_normals,
  const std::vector<float> &in_texcoords,
  ...
  MaterialReader& readMatFn)
{
  ...
  while (inStream.peek() != -1) {
    ...
    // normal //psdeo: there are no vertex normals in the cube.obj
    ...
    // texcoord	//psdeo: there are no text coordinates in cube.obj
    ...
    // object name	//psdeo: since there is only one object in cube.obj, no name is present
